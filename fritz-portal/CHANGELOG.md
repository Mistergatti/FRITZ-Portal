...
- Neu (i18n, src/i18n/index.tsx): vollständige Internationalisierung mit Wörterbüchern `de`/`en` (~200 Keys), `useI18n()`‑Hook, `I18nProvider`, `t(key, params?)` mit `{platzhalter}`‑Interpolation und persistenter Auswahl im `localStorage` (Schlüssel `lang`). Initiale Sprache aus `navigator.language` (EN für englische Browser, sonst DE).
- Neu (Sprach‑Toggle): Die `DE`/`EN`‑Pill im Header schaltet die Oberfläche live um – alle Navigations‑Labels, Statusleiste, Pages (Dashboard, Geräte, Detail, Traffic, Telefonie, SmartHome, System, Login‑/Loading‑Screen), Modale, Buttons, Tabellen‑Header, Filter‑Pillen, Status‑Messages und HA‑Sensors‑Block sind übersetzt.
...
- Neu (Geräteliste + Mesh): Anschlussdetails werden nach Möglichkeit angezeigt – `LAN 2 → 1 Gbit/s`, `WLAN 2,4 GHz → 58 Mbit/s` usw. Quelle ist das netDev-Feld `conn_info`/`ipinfo` (von FritzOS vorformatiert); wenn nicht vorhanden, wird Port/Band + Speed aus `port_name`, `wlan_band`, `conn_speed`, `txrate` rekonstruiert. In der Mesh-Box passt die Namens-Länge sich dynamisch an die Badge-Breite an.
